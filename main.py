...
class NonUPPFileProcessor(FileProcessor):
    ...
    def process_file(self, file_path: Path) -> pd.DataFrame:
        ...
        if df.empty:
            raise RegisterProcessingError(Fore.RED + f"Отчет по проводкам 1с пустой в файле {file_path.name}, обработка невозможна. Файл не УПП")
        return df
...
class FileProcessorFactory:
    ...
    def get_processor(file_path: Path) -> FileProcessor:
        ...
        if type_register_1C == 'UPP':
            ...
            raise RegisterProcessingError(f"Файл {file_path.name} не является корректным Отчетом по проводкам из 1С.")
    ...
    def _is_upp_or_not_upp_file(file_path: Path) -> str:
        ...
        upp_keys = {'дата', 'документ', 'содержание', 'дт', 'кт', 'сумма', 'субконто дт', 'субконто кт'}
...
class FileHandler:
    ...
    def _process_directory(self, dir_path: Path) -> None:
        ...
        try:
            ...
            if upp_results or non_upp_results:
                ...
                cols = df_pivot_upp.columns.tolist()
                first_cols = cols[:6]  # Первые 6 колонок
                cols_to_sort = cols[6:]  # Остальные колонки
                
                # Сортируем только нужные колонки
                cols_to_sort_sorted = sorted(cols_to_sort)
                
                # Собираем обратно DataFrame
                df_pivot_upp = df_pivot_upp[first_cols + cols_to_sort_sorted]
                                
                self._save_combined_results(df_pivot_upp, df_pivot_non_upp)
            else:
                raise NoRegisterFilesFoundError(Fore.RED + 'В папке не найдены Отчеты по проводкам 1С')
        finally:
    ...
    def _save_and_open_result(self, df: pd.DataFrame) -> None:
        ...
        if self.verbose:
            print('\nЗаписываем обработанный регистр в файл...\n')
...
class UserInterface:
    ...
    def get_input() -> list[Path]:
        ...
        print(Fore.YELLOW + "\nПеретащите файл отчета по проводкам 1С (.xlsx) или папку и нажмите Enter:")
